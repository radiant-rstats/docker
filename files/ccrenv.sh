...
function conda_create_r_kernel() {
    ...
    conda activate $1
    conda install -y -c conda-forge r-irkernel "${@:2}"
    ipython kernel install --user --name=$1
    conda deactivate
}

if [ "$1" != "" ]; then
    conda_create_r_kernel $1 ${@:2}
    echo "The code run in this function is:"
    declare -f conda_create_r_kernel
    echo "You may need to refresh your browser to see the new R-kernel icon for environment '$1'"
else
