...
if [ "${has_docker}" == "" ]; then
  ...
  docker run --net ${LABEL}_default -d -p 8080:8080 -p 8787:8787 -p 8989:8989 -v ${HOMEDIR}:/home/rstudio ${IMAGE}
  ...
  show_service () {
    ...
    if [ -z "${startup}" ]; then
      ...
      echo "-----------------------------------------------------------------------"
      ## based on https://stackoverflow.com/a/52852871/1974918
      docker run --net ${LABEL}_default -d -p 8080:8080 -p 8787:8787 -p 8989:8989 -v ${HOMEDIR}:/home/rstudio ${IMAGE}:${VERSION}
      echo "-----------------------------------------------------------------------"
    elif [ ${startup} == 7 ]; then
